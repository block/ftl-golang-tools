...
// To run:
//
// $ cd $GOPATH/src/github.com/worstell/x/tools/gopls
...
func validateGoModFile(goplsDir string) error {
	...
	hash := string(stdout)
	// Find the github.com/worstell/x/tools require line and compare the versions.
	var version string
	for _, req := range gomod.Require {
		if req.Mod.Path == "github.com/worstell/x/tools" {
			version = req.Mod.Version
			break
		}
	}
	if version == "" {
		return fmt.Errorf("no require for github.com/worstell/x/tools")
	}
	...
	if !strings.HasPrefix(hash, last) {
		return fmt.Errorf("github.com/worstell/x/tools pseudoversion should be at commit %s, instead got %s", hash, last)
	}
	return nil
}
