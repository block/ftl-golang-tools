...
why the gopls application depends on the cmp package:
	...
	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps github.com/TBD54566975/golang-tools/gopls |
		digraph somepath github.com/TBD54566975/golang-tools/gopls github.com/google/go-cmp/cmp

Show which packages in x/tools depend, perhaps indirectly, on the callgraph package:

	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps github.com/TBD54566975/golang-tools/... |
		digraph reverse github.com/TBD54566975/golang-tools/go/callgraph
