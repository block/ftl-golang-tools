...
why the gopls application depends on the cmp package:
	...
	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps github.com/worstell/x/tools/gopls |
		digraph somepath github.com/worstell/x/tools/gopls github.com/google/go-cmp/cmp

Show which packages in x/tools depend, perhaps indirectly, on the callgraph package:

	$ go list -f '{{.ImportPath}} {{join .Imports " "}}' -deps github.com/worstell/x/tools/... |
		digraph reverse github.com/worstell/x/tools/go/callgraph
